...
"""],
    ...
    def test_cycle_report_metrics_block_round_trips(self) -> None:
        """A report's metrics block must be readable by the trend, not just written.

        iterate_cycle writes a ```json fence inside an HTML comment; the fence
        is ```json, not ```. Slicing at the opening backticks alone leaves the
        language tag in the payload and the block parses as empty — which
        silently zeroes the trend for every report ever written.
        """
        with _scratch_iteration_state() as (scratch_state, scratch_reports):
            env = _iterate_env(scratch_state, scratch_reports)
            begin = run_script(
            ...
            finish = run_script(
                "scripts/iterate_cycle.py", "--json", "--finish", "--reject",
                "--reason", "component is not installed", env=env)
            self.assertEqual(finish.returncode, 0, finish.stderr + finish.stdout)

            # The report exists and declares a metrics block...
            report = scratch_reports / f"cycle-{cid:03d}.md"
            text = report.read_text(encoding="utf-8")
            self.assertIn("iterate_metrics", text)
            # ...and the trend actually reads it back.
            from scripts import trend_report
            state = json.loads(scratch_state.read_text(encoding="utf-8"))
            cycle = state["cycles"][-1]
            metrics = trend_report._report_metrics(state, cycle)
            self.assertIn("pages", metrics,
                          "metrics block was written but did not round-trip "
                          "through the trend parser")
            self.assertIsInstance(metrics["pages"], int)
    ...
    def test_trend_target_is_part_of_all(self) -> None:
        """trend.md must be regenerated by `make all`, not by hand.

        The artifact is derived from iteration-state.json; if it is not
        refreshed on every gate run it silently drifts until someone notices
        the trend disagreeing with the state it describes — which is exactly
        how it went stale when the target existed but was unwired.
        """
        makefile = (ROOT / "Makefile").read_text(encoding="utf-8")
        ...
        for line in makefile.splitlines():
            if line.startswith("all:"):
                self.assertIn("trend", line,
                              "trend must be a prerequisite of `all` so the "
                              "artifact is regenerated on every gate run")
                break
